...
def event_path(shock,rebound_frac,second_frac,second_day,noise_vol=.018):
    r=np.zeros(HORIZON);r[0]=shock
    reb=max(0.0,rebound_frac*abs(shock));daily_reb=(1+reb)**(1/5)-1;r[1:6]=daily_reb
    if second_frac>0:
        sec=-second_frac*abs(shock);d=(1+sec)**(1/4)-1;st=int(np.clip(second_day,6,HORIZON-5));r[st:st+4]=d
    for i in range(1,HORIZON):
        if r[i]==0:
            sig=noise_vol*(1.8 if i<10 else (1.3 if i<20 else 1.0));r[i]=RNG.normal(0.0004,sig)
...
def simulate_one(state_kind,hedge_mode):
    ...
    row=cand.iloc[int(RNG.integers(0,len(cand)))];dt=row.name;q0=float(row.q);gate0=bool(row.gate);d0=float(row.dist);prev=prior_returns(dt)
    if prev is None:return simulate_one(state_kind,hedge_mode)
    shock=-float(np.clip(RNG.normal(.155,.055),.06,.30));rebound=float(np.clip(RNG.normal(.75,.35),0,1.5));second=float(np.clip(RNG.normal(.45,.30),0,1.2)) if RNG.random()<.72 else 0.0;second_day=int(RNG.integers(6,16))
    ndx=event_path(shock,rebound,second,second_day,float(RNG.uniform(.012,.028)));beta=float(RNG.uniform(.55,.82));spr=beta*ndx+RNG.normal(0,.004,size=HORIZON);sp_rel=(1+d0)*np.cumprod(1+spr);gate_path=sp_rel<1.0;qld=np.clip(2*ndx-(drag_ann/252),-.99,.75)
    ...
    for i in range(1,min(12,HORIZON)):
        if np.sign(ndx[i])!=np.sign(ndx[i-1]) and abs(ndx[i])+abs(ndx[i-1])>.04:mf[i]-=float(RNG.uniform(.005,.02))
    hb0=min(H_CAP,max(0,1-q0));w=np.array([q0,.5*hb0,.5*hb0,max(0,1-q0-hb0)],float);nav=1.;peak=1.;mdd=0.;rets=[];hist=list(prev);cur_asym=recover_asym(q0,gate0);last_q=q0;q_targets=[q0];trade_count=0;day_under30=None;gate_day=None
    for i in range(HORIZON):
        if i>0:
            cur_asym,last_q,_=next_q_state(np.array(hist),cur_asym,last_q,bool(gate_path[i-1]));qtar=float(last_q);hb=min(H_CAP,max(0,1-qtar));target=np.array([qtar,.5*hb,.5*hb,max(0,1-qtar-hb)],float)
            if np.max(np.abs(w-target))>TRADE_BAND:w=target.copy();trade_count+=1
        ...
        a=np.array([qld[i],gold[i],mf[i],cash[i]]);rp=float(w@a);nav*=1+rp;rets.append(rp);peak=max(peak,nav);mdd=min(mdd,nav/peak-1);end=w*(1+a);w=end/end.sum();hist.append(qld[i]);hist=hist[-WIN:]
...
def summarize(arr):
    df=pd.DataFrame(arr)
    return {'N':len(df),'avg_q0':float(df.q0.mean()),'med_shock':float(df.shock.median()),'MDD_median':float(df.mdd.median()),'MDD_p10':float(df.mdd.quantile(.10)),'MDD_p05':float(df.mdd.quantile(.05)),'MDD_p01':float(df.mdd.quantile(.01)),'MDD_worst':float(df.mdd.min()),'P_MDD_lt20':float((df.mdd<-.20).mean()),'P_MDD_lt30':float((df.mdd<-.30).mean()),'P_MDD_lt40':float((df.mdd<-.40).mean()),'P_MDD_lt50':float((df.mdd<-.50).mean()),'Day0_median':float(df.day0.median()),'Day0_p05':float(df.day0.quantile(.05)),'Final40d_median':float(df.final.median()),'Final40d_p05':float(df.final.quantile(.05)),'q1_median':float(df.q1.median()),'q5_median':float(df.q5.median()),'P_q_under30_by_day2':float((df.under30<=2).mean()),'P_gate_by_day2':float((df.gate_day<=2).mean()),'Trades40d_med':float(df.trades.median())}
...
for state in ['UNCONDITIONAL','CALM_Q60','MAX_Q80']:
    for hedge in ['NEUTRAL','ADVERSE','SEVERE']:
        arr=[simulate_one(state,hedge) for _ in range(N)];print('MC',state,hedge,json.dumps(summarize(arr)))
...
def deterministic(q0,shock,hedge_mode):
    ...
    ndx=event_path(shock,.80,.50,9,.012);spr=.70*ndx;sp_rel=(1+d0)*np.cumprod(1+spr);gate=sp_rel<1;qld=np.clip(2*ndx-drag_ann/252,-.99,.75);gold=np.zeros(HORIZON);mf=np.zeros(HORIZON);cash=np.zeros(HORIZON)
